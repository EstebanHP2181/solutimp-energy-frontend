...
/** Normaliza respuesta JSON (snake_case y alias frecuentes). */
export function normalizeSimulationPayload(raw: unknown): SimulationResult | null {
  if (!raw || typeof raw !== 'object') return null
  const r = raw as Record<string, unknown>
  const econ = (r.economics as Record<string, unknown>) || r
  ...
  return {
    power_kwp: num(r, 'power_kwp', 'kwp', 'potencia_kwp'),
    panels: num(r, 'panels', 'num_panels', 'cantidad_paneles'),
    panel_model: str(r, 'panel_model', 'modelo_panel'),
    inverter: str(r, 'inverter', 'inversor'),
    includes_battery: bool(r, 'includes_battery', 'bateria_incluida', 'battery_included'),
    economics,
    backup,
